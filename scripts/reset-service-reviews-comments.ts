/**
 * Reset Service Reviews and Comments
 * 
 * This script resets all review and comment counts for all services to 0.
 * Optionally, it can also delete the actual Review, Comment, and QuickRating records.
 * 
 * Usage:
 *   npm run db:reset-service-reviews
 *   npm run db:reset-service-reviews -- --delete-reviews
 *   npm run db:reset-service-reviews -- --delete-comments
 *   npm run db:reset-service-reviews -- --delete-ratings
 *   npm run db:reset-service-reviews -- --delete-reviews --delete-comments --delete-ratings
 * 
 * Options:
 *   --counts-only      Only reset counts, don't delete records (default if no flags)
 *   --delete-reviews   Delete all Review records for services
 *   --delete-comments  Delete all Comment records for services
 *   --delete-ratings   Delete all QuickRating records for services
 * 
 * What gets reset:
 *   - totalReviews: 0
 *   - positiveReviews: 0
...
async function resetServiceReviewsAndComments(options: ResetOptions = {}) {
  const {
  ...
  console.log('🔄 Starting Service Reviews and Comments Reset');
  ...
  try {
    ...
    for (const service of services) {
      console.log(`Processing: ${service.serviceName} (${service.id})`);
    ...
    console.log('='.repeat(60));
    console.log(`📊 Summary:`);
    console.log(`   - Services processed: ${processedCount}`);
    if (deleteReviews) {
...
async function main() {
  ...
  await resetServiceReviewsAndComments(options);
}
