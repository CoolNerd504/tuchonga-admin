import type { Prisma, ItemType, ReactionType } from '@prisma/client';
...
export interface CommentFilters {
  ...
  page?: number;
  limit?: number;
  sortBy?: 'createdAt' | 'agreeCount' | 'disagreeCount' | 'replyCount';
  sortOrder?: 'asc' | 'desc';
  hasReplies?: boolean;
  search?: string;
}
...
export const commentServicePrisma = {
  ...
  async getAllComments(filters: CommentFilters = {}) {
    const {
      itemId,
      itemType,
      userId,
  ...
  async getServiceComments(serviceId: string, filters: CommentFilters = {}) {
    return this.getAllComments({
      ...filters,
      itemId: serviceId,
      itemType: 'SERVICE',
  ...
  async getCommentReplies(parentId: string, filters: CommentFilters = {}) {
    return this.getAllComments({
      ...filters,
      parentId,
    });
  ...
  async deleteComment(id: string, userId: string, isAdmin = false) {
    const comment = await prisma.comment.findUnique({
      where: { id },
    });
  ...
  async reactToComment(commentId: string, userId: string, reactionType: ReactionType) {
  ...
  async updateUserCommentAnalytics(userId: string, itemType: ItemType) {
    const incrementField =
      itemType === 'PRODUCT' ? { productComments: { increment: 1 } } : { serviceComments: { increment: 1 } };
  ...
  async updateUserReactionAnalytics(userId: string, reactionType: ReactionType) {
    const incrementField =
      reactionType === 'AGREE' ? { totalAgrees: { increment: 1 } } : { totalDisagrees: { increment: 1 } };
