...
router.get('/count', verifyToken, async (req, res) => {
  try {
    ...
    const count = await favoriteServicePrisma.getFavoriteCount(userId, itemType as any);
...
router.get('/top/products', verifyToken, verifyAdmin, async (req, res) => {
  try {
    ...
    const result = await favoriteServicePrisma.getMostFavoritedProducts(
...
router.get('/top/services', verifyToken, verifyAdmin, async (req, res) => {
  try {
    const { limit } = req.query;
    const result = await favoriteServicePrisma.getMostFavoritedServices(
      limit ? parseInt(limit as string) : 10
    );
